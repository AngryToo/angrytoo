...
class Entity {
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            if (killText === 'You have been kille') killText = 'You have a suisade';
            this.sendMessage(killText + '.');
            // If I'm the leader, broadcast it:
            if (this.id === room.topPlayerID) {
                let usurptText = (this.name === '') ? 'The leader': this.name;
                if (notJustFood) { 
                    usurptText += ' gets the lead and been usurped by';
                    killers.forEach(instance => {
                        usurptText += ' ';
                        usurptText += (instance.name === '') ? 'unnamed player' : instance.name;
                        usurptText += ' and';
                    });
        ...
        connect: (() => {
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    body.sendMessage('thats it');
                    body.sendMessage('welcome to angry too arras');
...
var maintainloop = (() => {
    ...
    let spawnBosses = (() => {
        ...
        return census => {
            if (timer > 300 && ran.dice(5 - timer)) {
                util.log('[SPAWN] Preparing to spawn...');
                timer = 0;
                let choice = [];
                switch (ran.chooseChance(40, 1, 6, 5, 7, 7, 5, 3, 5, 7, 4 ,4 ,5)) {
                    case 0: 
                        choice = [[Class.fallenspread,Class.fallenmachinegunner,Class.fallenauto4, Class.fallenspray, Class.fallenmachine, Class.fallen_destroy], 2, 'a', 'nest'];
sockets.broadcast('hey brotehr');
                        break;
                    case 1: 
                        choice = [[Class.fallenassassin, Class.fallenspray, Class.fallen_nailgun, Class.fallenquint, Class.fallenmachine], 2, 'castle', 'norm']; 
                        sockets.broadcast('assass me go to ass');
                        break;
case 2: 
                        choice = [[Class.fallenguntrap, Class.fallen_nailgun], 2, 'castle', 'norm']; 
                        sockets.broadcast('lets go ewerybody');
                        break;
                    case 3: 
                        choice = [[Class.fallenbasic, Class.fallenmachine], 2, 'castle', 'norm']; 
                        sockets.broadcast('uhh. is that you?');
                        break;
                }
            ...
            // Bots
                if (bots.length < c.BOTS) {
                    ...
                    o.color = 18;
                      o.define(Class.bot);
                     let arrayOfClasses = [
          Class.basic,Class.ranger,Class.overlord,Class.twin,Class.penta,Class.autoover,Class.destroy, Class.anni, Class.pound
          

        ]; //put as many classes as you want made by oblivion plain;
let newClass =
          arrayOfClasses[Math.floor(Math.random() * arrayOfClasses.length)];
        o.define(newClass);
